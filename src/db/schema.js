export function applySchema(db) {
  ...
  // Seed de projetos conhecidos
  const knownProjects = [
    { name: 'Brandspace',       slug: 'brandspace',      path: '/config/workspace/brandspace',      claude_md: '/config/workspace/brandspace/CLAUDE.md',      dev_port: 8080 },
    { name: 'TrackingMachine',  slug: 'trackingmachine', path: '/config/workspace/trackingmachine', claude_md: '/config/workspace/trackingmachine/CLAUDE.md',  dev_port: 8081 },
    { name: 'Ralab',            slug: 'ralab',           path: '/config/workspace/ralab',           claude_md: null,                                          dev_port: 8082 },
    { name: 'FisioExpert',      slug: 'fisioexpert',     path: '/config/workspace/fisioexpert',     claude_md: null,                                          dev_port: 8083 },
    { name: 'ABCPrime',         slug: 'abcprime',        path: '/config/workspace/abcprime',        claude_md: null,                                          dev_port: 8084 },
    { name: 'Orion',           slug: 'orion',          path: '/config/workspace/orion',          claude_md: null,                                          dev_port: 8088 },
  ]
  ...
  const migrations = [
    ...
    // Último papel que respondeu: 'user' | 'assistant' — detecta "aguardando input"
    `ALTER TABLE claude_sessions ADD COLUMN last_msg_role TEXT`,
    // Rascunho persistido no banco: texto e anexos serializado como JSON
  ...
  // Índice composto para buscar mensagens ativas de uma sessão de forma eficiente
  try {
    db.exec(`CREATE INDEX IF NOT EXISTS idx_messages_session_active ON messages(session_id, active, created_at)`)
  } catch (_e) { /* ignora se já existir */ }

  // Índices para soft delete e categorias
  try {
  ...
  // Fact versions — histórico de versões para concept drift
  db.exec(`
  ...
  // Proactive questions — fila de perguntas estratégicas do active learner
  db.exec(`
  ...
  // Co-retrieval graph — associações automáticas entre memórias co-acessadas
  db.exec(`
  ...
  // ── Round 4 — 17 novas melhorias ─────────────────────────────────────────────

  // Drift log — rastreia quando o significado de um fato derivou
  db.exec(`
  ...
  // Tier2 summaries — sumarização abstractive por categoria (cache semanal)
  db.exec(`
  ...
  // Tier3 narratives — sumarização narrativa por tema (cache mensal)
  db.exec(`
  ...
  // Temporal events — índice de eventos temporais extraídos de memórias
  db.exec(`
  ...
  // Contradiction queue — fila de contradições para resolução ativa
  db.exec(`
  ...
  // Causal links — grafo causal (causa → efeito)
  db.exec(`
  ...
  // Skill bundles — grupos de skills ativadas juntas
  db.exec(`
    CREATE TABLE IF NOT EXISTS skill_bundles (
      ...
      -- JSON array de nomes de skills
      trigger_words TEXT NOT NULL DEFAULT '[]',
      -- JSON array de palavras que ativam o bundle
      usage_count   INTEGER NOT NULL DEFAULT 0,
  ...
  // Skill co-usage matrix — para collaborative filtering
  db.exec(`
  ...
  // Skill session usage — rastreia uso por sessão
  db.exec(`
  ...
  // Skill dependencies — dependências e conflitos entre skills
  ...
  // ── Round 5 — 11 novas melhorias ─────────────────────────────────────────────

  // H5: Cron suggestions — sugestões de cron jobs geradas por Haiku pós-sessão
  db.exec(`
  ...
  // U1: Task patterns — padrões de tarefa detectados (para síntese de skills)
  db.exec(`
  ...
  // U3: Dedup queue — fila de deduplicação fuzzy com votação
  db.exec(`
  ...
  // U4: Skill rejections — rastreia correções do usuário a sugestões de skills
  db.exec(`
  ...
  // U4: Skill patches — histórico de patches automáticos do critic loop
  db.exec(`
  ...
  // U5: Memory snapshots — estado de memórias por sessão (time-travel)
  db.exec(`
  ...
  // Academia — backlog de melhorias de arquitetura propostas na página
  db.exec(`
  ...
  // H3: Session lineage — rastreia rotação de session_id na compressão
  ...
  // H4: Cron jobs avançados — script pre-run, wake-gate, context_from, output log
  db.exec(`
  ...
  const cronAdvancedMigrations = [
    ...
    // Sessão-alvo: quando setado, o job INJETA task_prompt na sessão Claude Code
    // (via sendToSession/claude --resume) em vez de spawnar um claude -p pelado.
    // É o que faz "automação que cutuca a sessão X" funcionar de verdade.
  ...
  // H6: Call log — rastreia custo estimado por modelo e canal
  db.exec(`
  ...
  // H5: Paridade Hermes — schedule kinds, state machine, deliver multi-plataforma, workdir
  ...
  // H8: Orquestrador autônomo em loop (multi-agent orchestrator)
  db.exec(`
  ...
  // H7: Kanban multi-agente durável
  db.exec(`
  ...
  // Chat unificado: aprovações bloqueantes cross-surface + inbox de decisões autônomas
  db.exec(`
  ...
  // Adiciona user_id à tabela sessions (idempotente)
  ...
  // Avaliação do Orion por colaborador
