 ...
 * Blueprint catalog com templates pré-definidos disponíveis como base.
 */
...
// Templates pré-definidos (blueprint catalog)
export const BLUEPRINT_CATALOG = [
  { name: 'daily-standup',    schedule: '0 9 * * 1-5', task: 'Faça um standup resumido: o que estava em andamento ontem, o que planejo hoje, algum bloqueio?' },
  { name: 'weekly-review',    schedule: '0 18 * * 5',  task: 'Revisão semanal: o que aprendi, o que completei, o que está pendente?' },
  { name: 'skill-audit',      schedule: '0 10 * * 0',  task: 'Audite as skills ativas: alguma desatualizada, alguma padrão recorrente que deveria virar skill?' },
  { name: 'memory-health',    schedule: '0 3 1 * *',   task: 'Verifique saúde da memória: confiança média, contradições pendentes, categorias mais acessadas.' },
  { name: 'project-checkin',  schedule: '0 17 * * 3',  task: 'Check-in semanal dos projetos: status de Orion, Brandspace, ABCPrime. Algum risco ou bloqueio?' },
]

/**
 * Sugere cron jobs baseados no conteúdo de uma sessão.
 * @param {string} sessionId
 * @param {string} conversationSummary - resumo/últimas mensagens da sessão
 */
export async function suggestCronJobs(sessionId, conversationSummary) {
  ...
  try {
    ...
    // Lista blueprints já instalados para evitar duplicatas
...
Máximo 2 sugestões. Nomes concisos, tarefas em português.`
    ...
    // Salva como sugestões pendentes (usuário decide se ativa)
    ...
    // Notifica via WhatsApp se há sugestões
...
/** Ativa uma sugestão (cria o cron job real) */
