...
const MODEL_PRICES = {
  haiku:  { input: 0.25,  output: 0.75  },  // $ per 1M tokens
...
export async function checkInactivity() {
  const db = getDb()
  const ownerJid = process.env.WHATSAPP_OWNER_JID
  const threshold = 48 * 3600 // 48h sem atividade = alerta
  const now = Math.floor(Date.now() / 1000)

  const collabs = db.prepare(`SELECT id, display_name, username, whatsapp_jid FROM users WHERE role='collaborator' AND is_active=1`).all()
  for (const u of collabs) {
...
export function addTokenUsage(userId, inputTokens, outputTokens) {
  ...
  if (!total) return
  try {
    getDb().prepare(`UPDATE users SET tokens_this_month=COALESCE(tokens_this_month,0)+? WHERE id=?`).run(total, userId)
  } catch {}
}
