...
function clusterByKeyword(memories, maxClusters = 10) {
  ...
  for (const m of memories) {
    ...
    for (const other of memories) {
      ...
      if (shared >= 2) { cluster.push(other); used.add(other.id) }
    }
    if (cluster.length >= 2) { clusters.push(cluster); if (clusters.length >= maxClusters) break }
  }
  return clusters
}
...
export async function runPhase3() {
  ...
  for (const cluster of clusters) {
    ...
    saveMemory({ content: cleanContent, type: 'semantic', source: 'phase3', confidence: Math.min(0.9, avgConf + 0.1), sourceTool: 'phase3-merge' })
    for (const m of cluster) db.prepare(`DELETE FROM memories WHERE id = ?`).run(m.id)
  ...
  for (const m of skillCandidates) {
    ...
    if (!title) continue
    db.prepare(`INSERT INTO skills (name, content, confidence, usage_count) VALUES (?, ?, ?, ?) ON CONFLICT(name) DO UPDATE SET content = excluded.content, confidence = MAX(confidence, excluded.confidence), updated_at = unixepoch()`).run(title, skillContent, m.confidence, m.access_count)
    db.prepare(`UPDATE memories SET type = 'skill', updated_at = ? WHERE id = ?`).run(now, m.id)
    skills++
  }

  const vaultSkills = db.prepare(`SELECT * FROM skills WHERE confidence >= 0.9 AND vault_path IS NULL ORDER BY usage_count DESC LIMIT 5`).all()
  ...
  } catch (err) { console.warn('[phase3] contradiction scan erro:', err.message) }

  setImmediate(() => { refreshAllCategorySummaries().catch(() => {}) })
