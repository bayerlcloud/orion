 ...
 * Haiku verifica se fatos de decisão/pessoa/projeto ainda são verdade.
 * Fatos obsoletos são arquivados (soft delete) com registro do motivo.
 */
...
export async function runPhase6() {
  ...
  // 10 memórias de categorias críticas, alta confiança, sem verificação recente
  const facts = db.prepare(`
    ...
    LIMIT 10
  `).all(now - 60 * 86400)  // só verifica se faz mais de 60 dias

  for (const fact of facts) {
    const prompt = `Este fato ainda é verdade hoje (2026)?
"${fact.content}"

Considere: fatos sobre projetos, decisões, pessoas e ferramentas podem mudar com o tempo.
...
Retorne APENAS JSON: {"still_true": true|false, "reason": "justificativa curta"}`
    ...
    if (!stillTrue) {
      // Snapshot da versão antes de arquivar (concept drift tracking)
      snapshotMemoryVersion(fact.id, `ground_truth_false: ${reason.slice(0, 60)}`)
      db.prepare(`UPDATE memories SET archived = 1, metadata = ?, updated_at = ? WHERE id = ?`)
        .run(JSON.stringify(meta), now, fact.id)
      archived++
    } else {
