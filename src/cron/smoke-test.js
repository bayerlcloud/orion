 ...
 * Falha em qualquer item → alerta no WhatsApp. Resultado fica em data/smoke-last.json
 * e no endpoint /api/orion/smoke (painel).
 *
 * Motivação: o delegate ficou quebrado por meses (flag --system inexistente) sem
 * ninguém perceber. Coisas quebram em silêncio; este cron faz o silêncio gritar.
 */
...
export async function runSmokeTest() {
  ...
  // 1. CLI do claude + flag do delegate (o bug histórico)
  ...
  // 2. Memória: save → retrieve → cleanup
  // confidence alta: o retrieval descarta (corretamente) memórias abaixo do MIN_TRUST
  ...
  // 3. Evolution API viva (sem enviar mensagem — checa o estado da instância)
  ...
  // 4. Playwright MCP (o outro quebrado silencioso histórico)
  // node:http puro — o fetch/undici descarta o header Host silenciosamente,
  // e o container do playwright só aceita Host localhost
  ...
  // 5. Banco: escrita/leitura básica
  ...
  if (failed.length) {
    try {
      ...
      logger.error({ err }, 'não consegui alertar no WhatsApp (a própria Evolution pode ser a falha)')
    }
  }
  return result
}
