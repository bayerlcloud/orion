 ...
 * Haiku avalia qualidade das memórias mais acessadas.
 * Score < 2 → reduz confiança. Resultado enviado por WhatsApp.
 */
...
export async function runPhase5() {
  ...
  // Top 20 memórias mais acessadas e não auditadas recentemente (últimos 30 dias)
...
Retorne APENAS JSON: {"score": 1-5, "feedback": "motivo curto"}`
    ...
    if (score < 2) {
      ...
      db.prepare(`UPDATE memories SET confidence = ?, metadata = ?, updated_at = ? WHERE id = ?`)
        .run(newConf, JSON.stringify(meta), now, m.id)
      penalized++
    } else {
      db.prepare(`UPDATE memories SET metadata = ?, updated_at = ? WHERE id = ?`)
        .run(JSON.stringify(meta), now, m.id)
    }

    // Agenda próxima revisão com base no audit score (SM-2)
    scheduleNextReview(m.id, score)

    audited++
  }

  // ── Trust calibration: computa calibração de confiança atual ─────────────────
  ...
  try {
    ...
    logger.warn({ err: err.message }, '[phase5] calibration error')
  }
