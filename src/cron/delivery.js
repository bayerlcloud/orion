...
export function resolveTargets(deliver = 'whatsapp', origin = null) {
  ...
  for (const part of parts) {
    ...
    if (lower === 'all') {
      if (process.env.WHATSAPP_OWNER_JID) targets.push({ platform: 'whatsapp', jid: process.env.WHATSAPP_OWNER_JID })
      if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID) targets.push({ platform: 'telegram', chatId: process.env.TELEGRAM_CHAT_ID })
      if (process.env.WEBHOOK_CRON_URL) targets.push({ platform: 'webhook', url: process.env.WEBHOOK_CRON_URL })
      continue
    }
    if (lower === 'origin') {
      if (!origin) continue
      if (origin.platform === 'whatsapp' && origin.jid) targets.push({ platform: 'whatsapp', jid: origin.jid })
      else if (origin.platform === 'telegram' && origin.chatId) targets.push({ platform: 'telegram', chatId: origin.chatId, threadId: origin.threadId })
      else if (origin.platform === 'webhook' && origin.url) targets.push({ platform: 'webhook', url: origin.url })
      continue
    }
    if (lower === 'local') { targets.push({ platform: 'local' }); continue }
    if (lower === 'whatsapp' || lower.startsWith('whatsapp:')) {
      const jid = lower.startsWith('whatsapp:') ? part.slice(9) : process.env.WHATSAPP_OWNER_JID
  ...
  const seen = new Set()
  return targets.filter(t => { const key = JSON.stringify(t); if (seen.has(key)) return false; seen.add(key); return true })
}
...
export async function deliverResult(targets, jobName, jobId, output) {
  ...
  for (const target of targets) {
    try { await _deliverToTarget(target, jobName, jobId, output) } catch (err) {
...
async function _deliverToTarget(target, jobName, jobId, output) {
  ...
  switch (target.platform) {
    case 'whatsapp': await sendWhatsApp(target.jid, msg); break
    case 'telegram': await _sendTelegram(target.chatId, msg, target.threadId); break
    case 'webhook': await _sendWebhook(target.url, { jobName, jobId, output, timestamp: new Date().toISOString() }); break
    case 'local': await _saveLocal(jobId, jobName, output); break
    default: logger.warn({ platform: target.platform }, 'plataforma desconhecida, ignorando')
  }
}
...
async function _sendTelegram(chatId, text, threadId) {
  ...
  if (!res.ok) { const err = await res.text(); throw new Error(`Telegram ${res.status}: ${err.slice(0, 200)}`) }
}
