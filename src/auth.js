...
// Segredo persistente (HMAC stateless — sobrevive a restarts)
...
// Cria o owner padrão se a tabela users estiver vazia
export function bootstrapOwner() {
  const db = getDb()
  const { c } = db.prepare('SELECT COUNT(*) as c FROM users').get()
  if (c > 0) return
  const hash = hashPassword('Bayerl21!')
  db.prepare(`INSERT INTO users (username, display_name, role, password_hash, avatar_color)
    VALUES (?, ?, 'owner', ?, ?)`).run('bayerl', 'Danilo', hash, '#6366f1')
  console.log('[auth] Usuário owner "bayerl" criado com senha padrão.')
}

export function login(username, password) {
  const db = getDb()
  // Aceita login por username OU por email
  ...
  // Payload: userId.username.role.timestamp
...
export function getUser(req) {
  ...
  // Novo formato: id.username.role.timestamp (4 partes)
  // Formato legado: username.timestamp (2 partes) — invalida silenciosamente
...
// Uso: app.get('/rota-admin', requireOwner, handler)
