...
export async function handleChatCompletion(req, res) {
  ...
  if (stream) {
    ...
    let content = ''
    try { const parsed = JSON.parse(result.stdout); content = parsed.result ?? parsed.content ?? result.stdout }
    ...
    res.write(`data: ${JSON.stringify({ id: completionId, object: 'chat.completion.chunk', created, model: claudeModel, choices: [{ index: 0, delta: {}, finish_reason: 'stop' }] })}\n\n`)
    res.write('data: [DONE]\n\n')
    return res.end()
  }

  let result
  try { result = await execa('claude', claudeArgs, { cwd: '/config/workspace', timeout: 120_000 }) }
  ...
  res.json({
    id: completionId, object: 'chat.completion', created, model: claudeModel,
