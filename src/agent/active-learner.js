/**
 * Active Learner — perguntas estratégicas proativas.
 *
 * Depois de cada turno, verifica categorias de memória com baixa cobertura.
 * Quando encontra lacuna importante (person/decision/user_pref com < 3 memórias),
 * gera UMA pergunta natural via Haiku e enfileira em proactive_questions.
 *
 * A pergunta fica disponível em GET /api/memory/pending-question para
 * ser incluída no contexto do próximo turno quando oportuno.
 */
...
// Ordem de prioridade por importância da categoria
const PRIORITY_ORDER = ['person', 'decision', 'user_pref', 'project', 'tool', 'general']

// Mínimo de memórias para considerar uma categoria "coberta"
const COVERAGE_MIN = 3

// Não gera nova pergunta se já houver uma não respondida da mesma categoria
// E limita frequência: no máximo 1 por 6h por categoria
...
export function checkAndQueueQuestion(contextSnippet = '') {
  setImmediate(async () => {
    try {
      ...
      if (totalMemories < 5) return  // banco muito vazio — aguardar

      const lowCats = categoryCounts.filter(r => r.n < COVERAGE_MIN)
      if (lowCats.length === 0) return

      // Ordena por prioridade de categoria
      ...
      // Verificar se já tem pergunta pendente ou recente para esta categoria
...
/**
 * Retorna a próxima pergunta pendente (ou null).
 * Usado pelo contexto do próximo turno.
 */
