...
export async function createCheckpoint(label = 'manual') {
  ...
  try {
    await execa('git', ['commit', '-m', `checkpoint: ${label}`], { cwd: CHECKPOINT_REPO })
...
export async function listCheckpoints() {
  await ensureRepo()
  try {
    const { stdout } = await execa('git', ['log', `--max-count=${MAX_CHECKPOINTS}`, '--format=%H|%s|%ai'], { cwd: CHECKPOINT_REPO })
    if (!stdout.trim()) return []
    return stdout.trim().split('\n').map(line => { const [hash, subject, date] = line.split('|'); return { hash, subject, date } })
  } catch { return [] }
}
