...
export function extractTemporalEvents(text, referenceEpoch = null) {
  ...
  while ((m = relRe.exec(text)) !== null) {
    const epoch = resolveRelativeDate(m[0])
    if (epoch) events.push({ type: 'relative', text: m[0], epoch, precision: 'day' })
  }

  for (const { re, relation } of TEMPORAL_CONNECTIVES) {
    re.lastIndex = 0
    if (re.test(text)) events.push({ type: 'connective', relation, text: text.slice(0, 100) })
  }
...
export function indexMemoryEvents(memoryId, content, createdAt) {
  ...
  for (const ev of events) {
    try {
      db.prepare(`INSERT OR IGNORE INTO temporal_events (memory_id, event_type, event_text, epoch, precision, relation) VALUES (?, ?, ?, ?, ?, ?)`).run(memoryId, ev.type, ev.text ?? null, ev.epoch ?? null, ev.precision ?? null, ev.relation ?? null)
      saved++
    } catch {}
  }
  return saved
...
export function getMemoriesInPeriod(fromEpoch, toEpoch, limit = 20) {
  ...
  try {
    return db.prepare(`
      ...
      WHERE te.epoch BETWEEN ? AND ? AND m.archived = 0
      ORDER BY te.epoch DESC LIMIT ?
...
export function getChronologicalMemories(memoryIds) {
  ...
  try {
    return db.prepare(`
      ...
      FROM memories m LEFT JOIN temporal_events te ON te.memory_id = m.id
      WHERE m.id IN (${placeholders})
      GROUP BY m.id ORDER BY earliest_event ASC, m.created_at ASC
    `).all(...memoryIds)
  } catch { return memoryIds.map(id => ({ id })) }
}
