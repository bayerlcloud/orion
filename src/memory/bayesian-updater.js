 ...
 *
 * Posterior = clamp(prior + delta, 0.01, 0.99)
 */
...
/**
 * Aplica uma observação bayesiana a uma memória.
 * @param {string} memoryId
 * @param {keyof typeof DELTAS} observation
 * @param {object} [opts]
 * @param {string} [opts.reason] - texto explicativo para auditoria
 */
export function applyObservation(memoryId, observation, opts = {}) {
  ...
  db.prepare(`
    UPDATE memories
    SET confidence = ?, metadata = ?, updated_at = unixepoch()
    WHERE id = ?
  `).run(posterior, JSON.stringify(meta), memoryId)
...
/**
 * Detecta menções cruzadas de sessão e aplica bônus cross_session.
 * Roda em background — chame sem await.
 */
export async function applyCrossSessionBonus(content) {
  try {
    const db = getDb()
    // Busca memórias com conteúdo similar em SESSÕES DIFERENTES
    ...
    // Se há ≥2 sessões distintas, reforça confiança
...
/** Retorna o histórico de observações bayesianas de uma memória. */
