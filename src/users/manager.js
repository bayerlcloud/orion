...
export function listUsers() { return getDb().prepare(`SELECT ${PUBLIC_FIELDS} FROM users ORDER BY role DESC, created_at ASC`).all() }
export function getUserById(id) { return getDb().prepare(`SELECT ${PUBLIC_FIELDS} FROM users WHERE id = ?`).get(id) }
export function getUserByUsername(username) { return getDb().prepare(`SELECT ${PUBLIC_FIELDS} FROM users WHERE username = ?`).get(username) }
...
export function createUser({ username, display_name, email, role = 'collaborator', password, avatar_color = '#8b5cf6', notes }) {
  ...
  const result = db.prepare(`INSERT INTO users (username, display_name, email, role, password_hash, avatar_color, notes) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(username, display_name || username, email || null, role, hash, avatar_color, notes || null)
...
export function insertAuditLog({ user_id, username, method, path, ip, status_code, duration_ms, body_summary }) {
  try { getDb().prepare(`INSERT INTO audit_log (user_id, username, method, path, ip, status_code, duration_ms, body_summary) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(user_id ?? null, username ?? 'anon', method, path, ip ?? null, status_code ?? null, duration_ms ?? null, body_summary ?? null) } catch {}
}

export function getAuditLog({ userId, limit = 200, offset = 0, since } = {}) {
  const db = getDb(); const conds = [], vals = []
...
export function getTimesheet(userId, days = 30) {
  const db = getDb(); const since = Math.floor(Date.now() / 1000) - days * 86400
  ...
  const GAP = 15 * 60; const byDay = {}; let prev = null
  for (const { created_at } of logs) {
    const d = new Date(created_at * 1000).toISOString().slice(0, 10)
    if (!byDay[d]) byDay[d] = 0
    if (prev && created_at - prev <= GAP) byDay[d] += created_at - prev
    prev = created_at
  }
...
export function createTask(userId, { title, description, priority = 'normal', due_date, created_by }) {
  ...
  return getDb().prepare(`INSERT INTO collab_tasks (user_id, title, description, priority, due_date, created_by) VALUES (?,?,?,?,?,?)`).run(userId, title.trim(), description || null, priority, due_date || null, created_by || null).lastInsertRowid
}
...
export function updateTask(taskId, { title, description, status, priority, due_date }) {
  ...
  if (!fields.length) return
  fields.push('updated_at=unixepoch()'); vals.push(taskId)
  getDb().prepare(`UPDATE collab_tasks SET ${fields.join(',')} WHERE id=?`).run(...vals)
}

export function deleteTask(taskId) { getDb().prepare(`DELETE FROM collab_tasks WHERE id=?`).run(taskId) }

export function getUserPermissions(userId) { return getDb().prepare(`SELECT resource, allowed FROM user_permissions WHERE user_id=?`).all(userId) }
export function setUserPermission(userId, resource, allowed) { getDb().prepare(`INSERT INTO user_permissions (user_id, resource, allowed) VALUES (?,?,?) ON CONFLICT(user_id, resource) DO UPDATE SET allowed=excluded.allowed`).run(userId, resource, allowed ? 1 : 0) }
export function setUserPermissions(userId, permissions) {
  const db = getDb(); db.prepare(`DELETE FROM user_permissions WHERE user_id=?`).run(userId)
  for (const [resource, allowed] of Object.entries(permissions)) db.prepare(`INSERT INTO user_permissions (user_id, resource, allowed) VALUES (?,?,?)`).run(userId, resource, allowed ? 1 : 0)
}

export function getBudget(userId) { return getDb().prepare(`SELECT token_budget_monthly, tokens_this_month, budget_notified_at FROM users WHERE id=?`).get(userId) }
export function setBudget(userId, monthlyTokens) { getDb().prepare(`UPDATE users SET token_budget_monthly=?, tokens_this_month=0, budget_notified_at=NULL WHERE id=?`).run(monthlyTokens || null, userId) }
